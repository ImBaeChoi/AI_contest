...
from PyQt5.QtCore import Qt, QPropertyAnimation, QRect, QRectF, QTimer,QEasingCurve, pyqtSlot
...
LIGHT_MODE_STYLE = """
    ...
    QLabel {
        background: qlineargradient(x1: 0, y1: 0, x2: 0, y2: 1, 
                                    stop: 0 #FAB9DC, stop: 1 #7FEBFA); /* 밝은 그라데이션 */
        color: black;  /* 텍스트 색상 */
    }
    #textlabel_2, #textlabel_3, #textlabel_4 {
...
form_class = uic.loadUiType("data/win.ui")[0]  # 메인 UI 로드
options_dialog_form_class = uic.loadUiType("data/options_dialog.ui")[0]  # 옵션 UI 로드
...
class WindowClass(QMainWindow, form_class):
    def __init__(self):
        ...
        # 애니메이션 리스트 초기화
        self.animations = []  # 애니메이션 객체를 저장할 리스트

        self.apply_external_font("textEdit", "data/EliceDigitalCodingverH_Regular.ttf", font_size=15)
        self.apply_external_font("textEdit_2", "data/EliceDigitalCodingverH_Regular.ttf", font_size=15)
        self.apply_external_font("textEdit_3", "data/EliceDigitalCodingverH_Regular.ttf", font_size=15)
        ...
        """
        self.setStyleSheet(self.styleSheet() + placeholder_style)
        
        self.textEdit.setPlaceholderText("음성 인식 대기 중...")
        self.textEdit_2.setPlaceholderText("명령어 대기 중...")
        self.textEdit_3.setPlaceholderText("AI 응답 대기 중...")
    ...
    def animate_widgets(self): 
        ...
        for widget in widgets:
            ...
            if widget in [self.hidden_mic_btn, self.loopAni, self.text_label, self.loopAni_label]:
                animation.setEndValue(QRect(x, y_start - 500, width, height))
            else:
                animation.setEndValue(QRect(x, y_start - 400, width, height))
    ...
    def update_user_text(self, user_text):
        """사용자 질문을 텍스 창에 표시"""
        self.textEdit.clear()  # 첫 번째 텍스트 창의 내용을 지움
        self.textEdit_2.clear()  # 두 번째 텍스트 창의 내용을 지움
        self.textEdit_3.clear()
        self.textEdit.append(f"{user_text}")  # 첫 번째 텍스트 창에 사용자 질문 표시

    def update_terminal_text(self, terminal_text):
        """터미널 명령어를 텍스트 창에 표시"""
        self.textEdit_2.clear()  # 두 번째 텍스트 창의 내용을 지움
        self.textEdit_2.append(f"{terminal_text}")  # 두 번째 텍스트 창에 터미널 명령어 표시

    def update_ai_text(self, ai_text):
        """AI 응답을 텍스트 창에 표시"""
        self.append_ai_text(ai_text)  # AI 응답을 한 글자씩 추가

    def append_ai_text(self, char):
        """AI 텍스트 창에 한 글자씩 추가"""
        self.textEdit_3.insertPlainText(char)
    ...
    def update_toolbutton_icon(self):
        if self.current_mode == "light":
            self.toolButton.setIcon(QIcon("data/light_mode_icon.png"))
        else:
            self.toolButton.setIcon(QIcon("data/dark_mode_icon.png"))
    ...
    @pyqtSlot(str, str)
    def show_command_output(self, command, output_content):
        """명령어 실행 결과를 새로운 창에 표시"""
        output_window = QDialog(self)
        output_window.setWindowTitle(f"명령어 실행 결과: {command}")
        output_window.setGeometry(100, 100, 600, 400)

        output_text = QTextEdit(output_window)
        output_text.setGeometry(10, 10, 580, 380)
        output_text.setReadOnly(True)
        output_text.setText(output_content)
...
class CustomDialog(QDialog, options_dialog_form_class):
    ...
    def toggle(self, button, sbtn):  # 옵션 버튼 함수
        ...
        if button == self.toggle_btn1:
            self.toggle_mode()
        elif button == self.toggle_btn2:  # mute 버튼
            self.main_window.toggle_mute(button.isChecked())  # mute 상태 전달

    def toggle_mode(self):  # 다크 모드/라이트 모드 전환
        if self.current_mode == "light":
            self.setStyleSheet(DARK_MODE_STYLE)  # 크 모드로 변경
...
if __name__ == "__main__":
    ...
    app.setWindowIcon(QIcon("data/icon.ico"))  # 애플리케이션 아이콘 설정
